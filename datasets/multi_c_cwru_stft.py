...
from datasets.imgDatasets import dataset
...
def data_load(filename, axisname, label):
    ...
    fl = fl.reshape(-1, )
    while end <= fl.shape[0]: # 向下取整，最后不足1024的点丢弃
        x = fl[start:end]
...
class CWRU_STFT(object):
    ...
    def data_split(self, transfer_learning=True):
        ...
        if transfer_learning:
            ...
            # source_train = dataset(list_data=train_pd, transform=None)
            # source_val = dataset(list_data=val_pd, transform=None)
            # source_test = dataset(list_data=test_pd, transform=None)
            ...
            # target_train = dataset(list_data=train_pd, transform=None)
            # target_val = dataset(list_data=val_pd, transform=None)
